...
\end{equation}
which measures the average information content per symbol and sets a lower bound on the compressed size achievable by any lossless scheme. A string with high entropy has symbols distributed close to uniformly, which offers little redundancy for LZW to exploit. A low-entropy string has a skewed distribution, producing longer repeated substrings and faster dictionary saturation.
 
\citet{KosarajuManzini99} formalise this relationship for LZW specifically, proving bounds on how well LZW's compression ratio tracks the zeroth-order empirical entropy $H_0$ of the input string. Crucially, they show that on low-entropy strings, those where $H_0 \rightarrow 0$, the compression ratio can be significantly worse than that predicted by the entropy alone. This finding has a direct implication for exercise generation: Shannon entropy is a useful but imperfect predictor of how an LZW execution will behave. A low-entropy string does not automatically produce a short, easy exercise; the sequential structure of the string matters independently.
...
Third, it must \emph{exhibit actual compression}: if the compressed output is longer than the input, the exercise undermines the conceptual point of the algorithm. The generator should therefore reject strings for which LZW fails to compress.
...
The key insight is that controlling difficulty requires all three dimensions to be managed together. Fixing only step count can produce exercises that are technically equivalent in length but very different in cognitive load. Fixing only entropy can produce exercises where step counts vary widely. A generator that targets specific ranges across all three dimensions is more likely to produce exercises that feel comparable to students sitting the same test.

\subsection{Compression versus Decompression}

Compression and decompression exercises test different aspects of student understanding and have different difficulty profiles. In compression, the student must identify the longest current dictionary match at each step, which requires maintaining awareness of the evolving dictionary. In decompression, the student must reconstruct the dictionary from the code sequence alone, which tests understanding of the algorithm's symmetry and the special case where a code is used before it is fully defined. Because the difficulty drivers differ, the generator treats compression and decompression as separate exercise types with separate difficulty controls, even though both operate on the same underlying alphabet and dictionary structure.
...
\begin{itemize}
    ...
    \item Generated exercises must satisfy the well-formedness conditions established in Section~3.1: the step count must fall within a configurable tractable range, and the compressed output must be shorter than the input.
...
\begin{itemize}
    \item The system will not provide an interactive graphical interface. The target users are educators generating papers in bulk; a command-line interface with configurable parameters is sufficient for this use case.
...
The attempt limit $M$ is a practical safeguard against the failure mode identified by \citet{Hoz21}: constraints that are too tight can cause the generator to run indefinitely. Setting $M$ to a large but finite value ensures the system reports failure cleanly rather than hanging.
...
The target step count is held constant across profiles so that both produce exercises requiring the same amount of written work. What differs is the cognitive character of that work: a high-entropy exercise requires the student to track an unpredictable sequence of matches, while a low-entropy exercise involves recognising repeated structure. This distinction is captured partly by Shannon entropy and partly by input variance.
...
Entropy is used in this design as a diagnostic metric rather than a primary filter. As \citet{KosarajuManzini99} show, the relationship between entropy and LZW execution behaviour is not tight for short strings: two strings with similar entropy can produce very different step counts. The primary difficulty controls are therefore step count and dictionary growth, with entropy logged alongside each exercise to allow post-hoc analysis of consistency within a batch.

\subsection{Input Variance}

Input variance is an additional difficulty signal not captured by Shannon entropy. A student tracing the LZW algorithm by hand is affected not only by the statistical regularity of the string but by its appearance. A string such as \texttt{AAACAAAC} contains obvious visual structure that a student will recognise and use to form expectations about the execution. A string such as \texttt{ACGTCAGT} appears irregular, requiring the student to engage with each step independently.

Input variance is operationalised as the number of distinct adjacent character pairs (bigrams) in the input string, normalised by the maximum possible number of distinct bigrams given the alphabet size. Formally, for a string $s$ of length $n$ over alphabet $\Sigma$:

\begin{equation}
    V(s) = \frac{|\{(s_i, s_{i+1}) : 1 \leq i < n\}|}{|\Sigma|^2}
\end{equation}

A string with high input variance has many distinct adjacent pairs, appearing irregular to a reader. A string with low input variance has few distinct pairs, appearing patterned. This measure is simple to compute, requires no simulation, and is independent of entropy: a low-entropy string can have either high or low input variance depending on whether the dominant character appears in varied contexts.
...
The final symbol in the input is always emitted using the dictionary size after all preceding entries have been added, since no new entry is created at the last step. This edge case is handled explicitly in the simulator and is reflected in the answer key format, where the last row has no entry in the ``string added'' column.
...
The central implementation challenge is producing strings whose LZW execution properties fall within a target band. As there is no closed-form relationship between a string's characters and its step count or dictionary growth, the approach taken is to generate candidates and reject those that do not meet the targets.

The generator seeds each candidate by first drawing all four symbols without replacement, guaranteeing that every generated string contains at least one occurrence of each character in the alphabet. This prevents bad cases where a symbol is absent entirely, which would reduce the effective alphabet size and break the assumption that the initial dictionary has four entries. The remaining characters are drawn with profile-dependent weights: uniform (0.25 each) for high entropy, and skewed (0.6, 0.2, 0.1, 0.1) for low entropy. The string is then shuffled to remove any ordering bias introduced by the sampling procedure.
...
After passing the streak test, the string is simulated using \texttt{simulate\_lzw}, which returns the final dictionary size and the list of output codes. A candidate is accepted if its step count and dictionary size are both within one of their respective targets, and if the compressed output is strictly shorter than the original string in symbols. If no candidate is accepted within 100,000 iterations, the loop exits and returns the most recent result as a best-effort fallback.
...
The compression simulator maintains a dictionary initialised with the four base symbols, then processes the input left to right, extending the current match greedily. When a character would extend the match beyond a known entry, the current match is emitted as a code, the new entry is added to the dictionary, and the match resets to the current character. The bit width used to encode output code is computed at the moment of emission as $\lceil \log_2(\text{dict size}) \rceil$, where dict size is the number of entries before the new entry is added. This matches the convention used in the lecture slides and ensures that the decoder can reconstruct the same bit widths from the dictionary alone.
...
The answer key, shown in Figure~\ref{fig:compression_answer}, uses the same layout but populates all rows with the correct values from the simulation trace. The header is marked ``Examiner Solution Key'' to distinguish it from the student-facing sheet.
...
Figure~\ref{fig:attempts_vs_length} shows an asymmetry between the two profiles that is not visible from the output metrics alone. The low entropy profile requires consistently around 10,000 to 20,000 generation attempts at all string lengths, while the high entropy profile requires far fewer at moderate lengths but spikes at $n = 30$. Low entropy strings are structurally harder to satisfy simultaneously on both step count and dictionary size, because their skewed character distribution tends to produce fewer distinct substrings, making the dictionary grow more slowly and step counts harder to hit. The spike for high entropy at $n = 30$ occurs because the combined tolerance on both targets becomes difficult to satisfy with strictly uniform character weights at longer lengths.
...
The most direct measure of whether exercises are equally difficult is step count, since each step corresponds to one row the student must complete in the table. Figure~\ref{fig:steps_vs_length} shows that both profiles track the $0.65n$ target closely across all tested lengths, with standard deviation bands that collapse almost to zero. Within a batch generated at a fixed length and profile, every exercise requires essentially the same number of written steps.

Table~\ref{tab:consistency} summarises the consistency metrics for a batch of 60 exercises generated at $n = 15$ for each profile. The coefficient of variation (CV) measures spread relative to the mean; a lower CV indicates more consistent difficulty.
...
The results for the first four questions were strongly positive. 80\% of participants rated the generated questions as Similar or Very Similar in difficulty to questions they had seen in class, with the remaining 20\% selecting Neither Similar nor Dissimilar. All five agreed or strongly agreed that the question tested their understanding of LZW principles, and all five found the randomly generated input string Clear or Very Clear to work with.

The fairness question produced the most mixed responses: two participants
Strongly Agreed that receiving different but equally difficult questions is fair, one was neutral, one Disagreed, and one Strongly Disagreed. The free-text responses suggest the split comes down to a distinction between varying the input string, which participants were broadly comfortable with, and varying the type of question or algorithm, which they were not. This is consistent with how the generator works: every student gets a different string but the same question type, algorithm, and table format. The negative responses likely reflect a general discomfort with differentiated assessment rather than a specific objection to this approach. Given the sample size of five, these results should be treated as indicative rather than conclusive.
...
Usable with three parameters or fewer & Met & Section~5.6 \\
...
The outcome of this work is the development of two Python-based tools that generate fully formatted LaTeX question and answer sheets fit for use in class tests. Both compression and decompression exercises are supported, including the decompression special case where a code is encountered before its dictionary entry is fully defined, a known teaching point that the generator can optionally target. Difficulty is controlled through step count and dictionary size targeting, with Shannon entropy and input variance tracked as diagnostic measures. Across 60 generated exercises per profile at $n = 15$, step count coefficient of variation was 0\%, meaning every exercise in a batch required exactly the same number of written steps. The two profiles were well-separated on Shannon entropy (means of \num{1.85} and \num{1.50} bits per symbol), and all generated exercises compressed successfully. A class test study with five COMPSCI2026 students (drawn from a pool of 18) found that generated exercises were rated as comparable in difficulty to class examples, clear to work with, and effective at testing understanding of the algorithm.

\section{Reflection}

One thing that only became clear after running the generator at scale was how many attempts the low entropy profile needs — consistently in the range of 10,000 to 20,000 per exercise. The tolerances and weight vectors were tuned empirically, which worked but meant a lot of iteration before the generator behaved reliably. With more time, it would have been worth analysing the distribution of LZW step counts for a given character weight vector upfront, so the acceptance criteria could be set more precisely from the start rather than adjusted through repeated testing.
...
The current command-line interface is designed for an educator generating a batch of exercises. A natural extension would be a lightweight web interface that allows educators to configure and download exercises without using a terminal. Beyond the educator use case, the generator could be adapted into a student practice tool, allowing students to generate and attempt unlimited exercises before a class test. This would give the generator a second purpose beyond assessment: supported self-study with automatically generated, verified practice questions.
...
Replacing the generate-and-filter approach with a constructive one would address the high attempt count for low entropy strings. Rather than sampling random strings and rejecting most of them, a constructive generator would build the string one character at a time, making choices that keep the running LZW execution within the target band. Because LZW depends on the full history of the string, a simple greedy strategy would not always succeed; backtracking or beam search would be more reliable, and would also make it easier to target the decompression special case directly rather than filtering for it.
