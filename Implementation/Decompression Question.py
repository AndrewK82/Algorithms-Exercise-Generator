...
def has_special_case(text):
    ...
    for c in text:
        ...
        if wc in dictionary:
            ...
            code = dictionary[w]
            if prev_code is not None and code == dict_size - 1:
                return True
...
def generate_sequence(target_length=14, entropy_type='high_entropy', seen=None, special_case=False):
    ...
    for _ in range(100000):
        ...
        # If special case is required, reject strings that do not trigger it
        if special_case and not has_special_case(text):
            continue
        ...
        if (abs(final_dict_size - target_dict_size) <= tolerance
            ...
            return text, bitstream, total_bits, steps, entropy_value, input_variance, final_dict_size

    return text, bitstream, total_bits, steps, calculate_shannon_entropy(text), calculate_input_variance(text), final_dict_size
...
def main():
    ...
    try:
        ...
        sc_input = input("Include special case decompression? (y/n): ").strip().lower()
        special_case = sc_input == 'y'

    except ValueError:
        entropy_type, length, num_questions, special_case = 'high_entropy', 14, 3, False

    seen = set()
    i = 1
    while i <= num_questions:
        print(f"\n--- Generating Question {i} ---")
        text, bitstream, total_bits, steps, entropy_value, input_variance, final_dict_size = generate_sequence(
            length, entropy_type, seen=seen, special_case=special_case
        )
        ...
        print(f"  Special Case:       {'Yes' if is_special else 'No'}")
