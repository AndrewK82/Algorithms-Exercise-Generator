...
def generate_smart_lzw_string(length, entropy_type):
    target_dict_size = len(SYMBOLS) + max(3, int(length * SCALING_FACTORS[entropy_type]))
    # Simplification for the stats loop: force a result
    ...
    while True:
        ...
        if abs(d_size - target_dict_size) <= 2: # Slightly wider margin for faster generation
            return text, d_size, compressed

# --- NEW ANALYTICS & PLOTTING ---
def run_lzw_study(min_len=8, max_len=50):
    ...
    for length in range(min_len, max_len + 1):
        ...
        results.append({
            ...
            "Dict Size": d_size,
            "Compression %": savings,
            "Efficiency": orig_bits / comp_bits # Ratio > 1 means it's helping
        })
...
plt.subplot(1, 2, 2)
sns.regplot(data=df, x="Length", y="Dict Size", color='purple')
plt.title("Dictionary Growth (Complexity Scaling)")
plt.grid(True, alpha=0.3)
